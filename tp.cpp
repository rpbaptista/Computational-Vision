...
void ICP(std::vector<Vec3> const & ps, std::vector <Vec3> const & nps,
        ...
        // initializating weights; for now 1
        std::vector<double> poids(ps.size());
        for (unsigned int ind = 0; ind < ps.size(); ++ind){
            poids[ind] = 1.0;
        }

        // initializating stuff
        Vec3 p_centroid(0,0,0);
        Vec3 q_centroid(0,0,0);
        ...
        for (unsigned int ind = 0; ind< ps.size(); ++ind){
          ...
          Vec3 p =  ps[ind];
          Vec3 p_trans;
          p_trans = rotation*p + translation;

          unsigned int knn = 1;
          ANNidxArray id_nearest_neighbors = new ANNidx[knn];
          ANNdistArray square_distances_to_neighbors = new ANNdist[knn];
          qsKdTree.knearest(p_trans, 1, id_nearest_neighbors, square_distances_to_neighbors);

          Vec3 q =  qs[id_nearest_neighbors[0]];
        ...
        p_centroid /= sum_weight;
        q_centroid /= sum_weight;

        // vector recentrees
        std::vector<Vec3> p_line =  ps;
        std::vector<Vec3> q_line =  qs;
        for (unsigned int ind =0; ind<ps.size(); ++ind){
          p_line[ind] -= p_centroid;
          q_line[ind] -= q_centroid;
        }

        // calculer matrix de covariancia
        for (unsigned int ind = 0; ind<ps.size(); ++ind){
          rotation += Mat3::tensor(p_line[ind],q_line[ind]);
        }
        // setting rotation
       rotation.setRotation();
       translation = q_centroid - rotation*p_centroid;
